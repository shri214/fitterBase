...
export const EqualBranch: React.FC = () => {
  ...
  const validateBranchInput = (
    ...
    if (selectedType === "oldFormula" && Branch.branchOd <= 0) {
      toast("Branch OD must be greater than 0 for oldFormula");
      return false;
    }
    ...
    if (selectedType === "newFormula") {
      if (Branch.HeaderOd <= 0 || Branch.branchOd <= 0) {
        toast(
          "Both Header OD and Branch OD must be greater than 0 for newFormula"
        );
        return false;
      }

      if (Branch.HeaderOd <= Branch.branchOd) {
        toast("Header OD  must be greater than branch Id for newFormula");
        return false;
      }
    }
    ...
    <Container>
      ...
      <ImageContainer>
        ...
        {result?.cl && (
          <InfoBox>
            <p>
              Center Line : <strong>{result?.cl} mm</strong>
            </p>
            <p>
